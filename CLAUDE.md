...
**CEP normalization is automatic — mappers do NOT need to call fill_city_state or lookup_cep_by_city.** `core/exporter.py` and `core/property_exporter.py` call `fix_record_cep(record)` (from `core/cep_lookup.py`) on every person and property record before writing the CSV. `fix_record_cep` applies: (1) strip non-digits; (2) normaliza `estado` via `normalize_estado_uf()` (nome completo → sigla); (3) if valid 8-digit non-placeholder CEP → `fill_city_state` to complete missing cidade/estado; (4) if CEP missing, incomplete (< 8 digits), or placeholder (`"00000000"`, `"99999999"`) → `lookup_cep_by_city(cidade, estado)` fallback (busca CEP com final `000` para a cidade/UF). Any `fill_city_state` / `lookup_cep_by_city` calls left inside individual mappers are harmless (results are cached) but redundant.

**Estado — sempre sigla de 2 letras:** `fix_record_cep` normaliza automaticamente nomes por extenso (ex: `"Minas Gerais"` → `"MG"`) via `normalize_estado_uf()` (em `core/cep_lookup.py`). Isso garante que o fallback `lookup_cep_by_city` funcione mesmo quando o sistema de origem entrega o nome completo do estado. A função também está disponível para uso direto nos mappers quando necessário.
